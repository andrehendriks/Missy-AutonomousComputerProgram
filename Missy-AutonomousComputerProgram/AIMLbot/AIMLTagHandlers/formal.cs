...
namespace AIMLbot.AIMLTagHandlers
{
    ...
    public class formal : AIMLbot.Utils.AIMLTagHandler
    {
        ...
        {
            if (this.templateNode.Name.ToLower(CultureInfo.CurrentCulture) == "formal")
            {
                StringBuilder result = new StringBuilder();
                if (this.templateNode.InnerText.Length > 0)
                {
                    string[] words = this.templateNode.InnerText.ToLower(CultureInfo.CurrentCulture).Split();
                    foreach (string word in words)
                    {
                        string newWord = word.Substring(0, 1);
                        newWord = newWord.ToUpper(CultureInfo.CurrentCulture);
                        if (word.Length > 1)
                        {
