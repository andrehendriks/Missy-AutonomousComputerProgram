...
namespace AIMLbot.AIMLTagHandlers
{
    ...
    public class gossip : AIMLbot.Utils.AIMLTagHandler
    {
        ...
        {
            if (this.templateNode.Name.ToLower(CultureInfo.CurrentCulture) == "gossip")
            {
                // gossip is merely logged by the bot and written to log files
                if (this.templateNode.InnerText.Length > 0)
                {
                    this.bot.writeToLog(Properties.Resource.String3 + this.user.UserID + Properties.Resource.String4 + this.templateNode.InnerText + Properties.Resource.String5);
                }
            }
            return string.Empty;
        }
