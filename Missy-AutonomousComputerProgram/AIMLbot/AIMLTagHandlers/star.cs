...
namespace AIMLbot.AIMLTagHandlers
{
    ...
    public class star : AIMLbot.Utils.AIMLTagHandler
    {
        ...
        {
            if (this.templateNode.Name.ToLower(CultureInfo.CurrentCulture) == "star")
            {
                if (this.query.InputStar.Count > 0)
                {
                    ...
                    else if (this.templateNode.Attributes.Count == 1)
                    {
                        if (this.templateNode.Attributes[0].Name.ToLower(CultureInfo.CurrentCulture) == "index")
                        {
                            try
                            {
                                int index = Convert.ToInt32(this.templateNode.Attributes[0].Value, CultureInfo.CurrentCulture);
                                index--;
                                ...
                                {
                                    this.bot.writeToLog(Properties.Resource.String16 + this.request.rawInput);
                                }
                            }
                            catch
                            {
                                this.bot.writeToLog(Properties.Resource.String17 + this.request.rawInput);
                            }
                        }
                    }
                }
                ...
                {
                    this.bot.writeToLog(Properties.Resource.String18 + this.request.rawInput);
                }
            }
            return string.Empty;
        }
