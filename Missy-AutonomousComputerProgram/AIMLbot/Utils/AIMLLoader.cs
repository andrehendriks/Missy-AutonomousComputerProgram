...
namespace AIMLbot.Utils
{
    ...
    public class AIMLLoader
    {
        ...
        /// <param name="CultureInfo">CurrentCulture</param>
        public void loadAIML(string path)
        {
            if (Directory.Exists(path))
            {
                // process the AIML
                this.bot.writeToLog(Properties.Resource.String6 + path);
                ...
                if (fileEntries.Length > 0)
                {
                    ...
                    this.bot.writeToLog(Properties.Resource.String7 + Convert.ToString(this.bot.Size, CultureInfo.CurrentCulture) + Properties.Resource.String8);
                }
                else
                {
                    throw new FileNotFoundException(Properties.Resource.String9 + path + Properties.Resource.String10);
                }
            }
            else
            {
                throw new FileNotFoundException(Properties.Resource.String11 + path + Properties.Resource.String12);
            }
        }
        ...
        {
            this.bot.writeToLog(Properties.Resource.String42 + filename);
        ...
        private void processCategory(XmlNode node, string topicName, string filename)
        {
            ...
            {
                throw new XmlException(Properties.Resource.String48 + filename);
            }
            if (object.Equals(null, template))
            {
                throw new XmlException(Properties.Resource.String49 + pattern.InnerText + Properties.Resource.String50 + filename);
            }
            ...
            if (categoryPath.Length > 0)
            {
                ...
                {
                    this.bot.writeToLog(Properties.Resource.String51 + categoryPath + Properties.Resource.String52 + template.OuterXml + Properties.Resource.String53 + filename);
                }
            }
            else
            {
                this.bot.writeToLog(Properties.Resource.String54 + categoryPath + Properties.Resource.String55 + template.OuterXml + Properties.Resource.String56 + filename);
            }
        }
